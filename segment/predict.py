    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for j, (*xyxy, conf, cls) in enumerate(reversed(det[:, :6])):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        # xcen,ycen,hl,wl = annotator.find_centerpolygon(segments[j], color=colors(c, True))
                        annotator.find_centerpolygon(segments[j],label, color=colors(c, True))
                        # # image size = 640*480
                        # print(xcen,ycen)
                        # ser.write(f'{xcen} {ycen}\n'.encode())
                        
                        # ser.write(("%s %s"%(0,0)).encode())
            ...
            if view_img:
                ...
                im0 = cv2.line(im0,(0,370),(1280,370),(255,0,0),2)
